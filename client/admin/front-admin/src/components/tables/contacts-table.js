...
class ContactTable extends HTMLElement {
  constructor () {
    super()
  ...
  render () {
    ...
    this.data.rows.forEach(element => {
      ...
      elementItemList.textContent = `título: ${element.title}`
  ...
  async renderRegisterButtons () {
    ...
    this.shadow.querySelector('.table').addEventListener('click', async (event) => {
      if (event.target.closest('.edit-button')) {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_URL}/api/admin/contacs/${id}`)
...
customElements.define('contacts-table-component', ContactTable)
